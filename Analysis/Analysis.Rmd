...
#fviz_nbclust(X2clust, method = "gap_stat", FUN=hcut)
```
```{r}
...
fviz_nbclust(X2clust, method = "silhouette", FUN=hcut) 
ggsave("figs/silhouette_stat.png", width = 4, height = 3)
```

```{r}

fviz_nbclust(X2clust, method = "wss", FUN=hcut)
...
k <- 3
clusters_new <- hcut(X2clust_new, k = k)$cluster
